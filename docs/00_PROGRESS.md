...
| FIX-YT-OAUTH-A1 | Migration 097 — add OAuth token storage columns to youtube_connections | ✅ | Created `supabase/migrations/097_youtube_oauth_tokens.sql` + `scripts/apply-migration-097.mjs`. Applied to live project `lzkzfqshnjvlzosnntfx`. Added 8 nullable no-default columns: access_token, refresh_token, token_expires_at, scopes (text[]), channel_id, channel_title, last_synced_at, last_sync_error. Live verification: exactly 12 columns (4 original unchanged), existing row unmodified, `npx tsc --noEmit` passes. |
| FIX-YT-OAUTH-A3 | `getStoredYouTubeToken()` in lib/youtube/client.ts — read + auto-refresh stored OAuth token | ✅ | Added `getStoredYouTubeToken(userId)` to `lib/youtube/client.ts` (service client read of encrypted token columns, decrypt via `lib/youtube/token-crypto.ts`, 5-minute expiry buffer, Google token refresh via `POST https://oauth2.googleapis.com/token`, re-encrypt + persist access_token/token_expires_at, optional new refresh_token). Existing `getYouTubeAccessToken` (Option A) left untouched. New test file `lib/youtube/getStoredYouTubeToken.test.ts` (mocked fetch + mocked service client): 10/10 tests pass; `npx tsc --noEmit` exit 0; eslint 0 errors 0 warnings. Not wired into any caller (future A-phase task). |
| FIX-YT-OAUTH-A4 | Rewrite `/api/youtube/connect` to build real Google OAuth authorization URL with signed state cookie | ✅ | Rewrote `app/api/youtube/connect/route.ts` (Option A POST → Option B GET). Uses existing `getSupabaseServerClient()` + `auth.getUser()` auth pattern (401 if unauthenticated). Generates 32-byte base64url state via `node:crypto` `randomBytes`, stores it in httpOnly/secure/sameSite=lax `yt_oauth_state` cookie (10-min expiry), builds `https://accounts.google.com/o/oauth2/v2/auth` URL with client_id from `GOOGLE_CLIENT_ID`, redirect_uri from `NEXT_PUBLIC_APP_URL \|\| "http://localhost:3000"` + `/api/youtube/callback`, scope `https://www.googleapis.com/auth/youtube`, access_type=offline, prompt=consent, state. 302 redirect. 500 if GOOGLE_CLIENT_ID/base URL unresolvable. No state verification (A5), no token refresh call, no new deps. Verified: `npx tsc --noEmit` exit 0; eslint clean; grep proofs for auth URL + absence of `exchangeCodeForSession`/`signInWithOAuth`. |
| FIX-YT-OAUTH-A5a | OAuth callback — state verification + Google token exchange (no DB write) | ✅ | Rewrote `app/api/youtube/callback/route.ts` (Option A `exchangeCodeForSession` → Option B direct Google token exchange). Verifies `yt_oauth_state` cookie against `state` query param (missing cookie/param/mismatch → redirect with error). Deletes cookie after match (single-use). Exchanges code via POST `https://oauth2.googleapis.com/token` with `client_id`/`client_secret`/`redirect_uri`/`grant_type=authorization_code`. Logs token metadata only (no token strings). `backfillFriendInvites` kept (depends only on user.email). Success redirect appends `?oauth_step=a5a_success`. No DB write (A5b). Verified: tsc 0, eslint 0, grep proofs for absence of `exchangeCodeForSession`/`youtube_connections`. |
| FIX-YT-OAUTH-A5a-PATCH1 | Add env var presence guard before Google token exchange | ✅ | Added guard in `app/api/youtube/callback/route.ts` before fetch to Google token endpoint: if `GOOGLE_CLIENT_ID` or `GOOGLE_CLIENT_SECRET` missing → `logger.error` + redirect `?youtube_error=server_misconfigured`. Removed `!` non-null assertions on both env vars (now narrowed by guard). 5 lines added, 2 lines changed. Verified: tsc 0, eslint 0. |
