-- ============================================================
-- Migration 024 — Fix all ambiguous column references in get_feed
-- ============================================================
-- PostgreSQL RETURNS TABLE output parameters shadow CTE columns.
-- Any unqualified reference to a name that matches an output parameter
-- becomes ambiguous when that column also exists in a CTE.
--
-- Fixes applied:
--   - DISTINCT ON (node_id)       → DISTINCT ON (with_tags.node_id)
--   - ORDER BY node_id              → ORDER BY with_tags.node_id
--   - SELECT share_count FROM nodes_sort_cache
--                                     → SELECT nsc_cur.share_count FROM nodes_sort_cache nsc_cur
--   - SELECT avg_rating FROM nodes_sort_cache
--                                     → SELECT nsc_cur.avg_rating FROM nodes_sort_cache nsc_cur
--   - WHERE node_id = p_cursor_node_id in subqueries
--                                     → WHERE nsc_cur.node_id = p_cursor_node_id
-- ============================================================
