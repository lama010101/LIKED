-- Migration 037 — Fix ambiguous language_code column reference in create_node_with_metadata
--
-- BUG-LANG-AMBIGUOUS: The function had bare `language_code` references in subqueries
-- that could match both `tag_translations.language_code` and the parameter `p_language_code`.
-- This migration prefixes all column references with their table names to resolve ambiguity.
--
-- Preserves SECURITY DEFINER (set by migration 035)
-- Preserves GRANT EXECUTE statements
