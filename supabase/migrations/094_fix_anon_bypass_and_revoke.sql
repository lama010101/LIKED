...
DECLARE
  ...
  v_now TIMESTAMPTZ := now();
BEGIN
  IF NOT (auth.role() = 'service_role' OR (auth.role() = 'authenticated' AND p_sharer_id IS NOT DISTINCT FROM auth.uid())) THEN
    RAISE EXCEPTION 'Caller does not match user_id' USING ERRCODE = 'P0003';
  END IF;
  IF NOT (auth.role() = 'service_role' OR (auth.role() = 'authenticated' AND p_sharer_id IS NOT DISTINCT FROM auth.uid())) THEN
    RAISE EXCEPTION 'Caller does not match user_id' USING ERRCODE = 'P0003';
...
-- set_node_deleted (resource-only: role-aware ownership)
...
-- delete_folder (resource-only: role-aware ownership)
...
-- add_node_to_folder (resource-only: role-aware ownership)
...
-- remove_node_from_folder (resource-only: role-aware ownership)
...
-- ── service-only: REVOKE EXECUTE FROM PUBLIC, anon, authenticated ──
REVOKE EXECUTE ON FUNCTION public.create_node_with_metadata(p_owner_id uuid, p_url text, p_text_content text, p_title text, p_thumbnail_key text, p_language_code text, p_tag_labels text[], p_description text, p_auto_folder_name text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.import_url(p_owner_id uuid, p_url text, p_title text, p_thumbnail_key text, p_language_code text, p_description text, p_new_tag_labels text[], p_existing_tag_ids uuid[], p_folder_id uuid, p_note text, p_auto_folder_name text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.hard_delete_node(p_node_id uuid, p_user_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.direct_share(p_sharer_id uuid, p_node_id uuid, p_target_user_id uuid, p_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.group_share(p_sharer_id uuid, p_node_id uuid, p_group_id uuid, p_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.share_folder(p_sharer_id uuid, p_folder_id uuid, p_target_user_ids uuid[], p_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.group_unshare(p_sharer_id uuid, p_node_id uuid, p_group_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.create_group(p_owner_id uuid, p_name text, p_member_ids uuid[]) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.delete_folder(p_folder_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.add_node_to_folder(p_node_id uuid, p_folder_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.remove_node_from_folder(p_node_id uuid, p_folder_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.move_folder(p_folder_id uuid, p_new_parent_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.set_custom_order(p_user_id uuid, p_scope_key text, p_node_ids uuid[]) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.create_tag_with_translation(p_color text, p_label text, p_lang text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.unshare_folder_op(p_folder_share_op_id text, p_requesting_user_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.upsert_rating(p_user_id uuid, p_node_id uuid, p_score numeric) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.update_node_title(p_user_id uuid, p_node_id uuid, p_title text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.update_display_name(p_user_id uuid, p_display_name text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.update_display_name(p_user_id uuid, p_display_name text, p_normalized text, p_old_display_name text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.update_avatar_key(p_user_id uuid, p_avatar_key text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.update_avatar_key(p_user_id uuid, p_avatar_key text, p_new_count integer, p_today text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.change_node_permission(p_cause_id uuid, p_requesting_user_id uuid, p_new_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.change_folder_permission(p_folder_id uuid, p_target_user_id uuid, p_requesting_user_id uuid, p_new_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.has_node_permission(p_user_id uuid, p_node_id uuid, p_required_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.has_folder_permission(p_user_id uuid, p_folder_id uuid, p_required_permission text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.get_node_permission(p_user_id uuid, p_node_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.get_folder_permission(p_user_id uuid, p_folder_id uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.get_nodes_in_folder(p_user_id uuid, p_folder_id uuid, p_sort text) FROM PUBLIC, anon, authenticated;

-- ── authenticated-needed: REVOKE EXECUTE FROM PUBLIC, anon ──
REVOKE EXECUTE ON FUNCTION public.get_feed(p_user_id uuid, p_language_code text, p_view text, p_friend_id uuid, p_folder_id uuid, p_group_id uuid, p_filter_tag_ids uuid[], p_filter_friend_ids uuid[], p_filter_folder_ids uuid[], p_search_query text, p_sort text, p_cursor_created_at timestamp with time zone, p_cursor_node_id uuid, p_limit integer, p_exclude_foldered boolean, p_custom_order_ids uuid[]) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_social_timeline(p_user_id uuid, p_language_code text, p_cursor_created_at timestamp with time zone, p_cursor_id uuid, p_limit integer) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_friend_bar(p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_user_folders(p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_folder_tree(p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_visible_node_by_id(p_user_id uuid, p_node_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_visible_nodes(p_user_id uuid, p_exclude_foldered boolean) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_visible_nodes(p_user_id uuid, p_sort text, p_view text, p_mine_filter text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_visible_tags(p_user_id uuid, p_language_code text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_node_friend_ratings(p_node_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.is_folder_admin(p_folder_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.is_group_admin(p_group_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.folder_is_accessible(p_folder_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.folder_is_owned(p_folder_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.group_is_member(p_group_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.group_is_owned(p_group_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.rename_folder(p_folder_id uuid, p_name text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_or_create_unsorted_folder(p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_or_create_named_folder(p_user_id uuid, p_name text, p_color text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.move_node_to_folder(p_node_id uuid, p_target_folder_id uuid, p_source_folder_id uuid, p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.create_folder_with_nodes(p_name text, p_parent_folder_id uuid, p_node_ids uuid[], p_user_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.set_node_deleted(p_node_id uuid, p_deleted boolean) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.create_folder(p_name text, p_parent_folder_id uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.upsert_card_position(p_node_id uuid, p_folder_id uuid, p_pos_x integer, p_pos_y integer, p_width integer, p_height integer) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.liked_tag_palette(p_index integer) FROM PUBLIC, anon;
