-- ============================================================
-- Migration 086 — AUDIT-06 P2-1/P2-12: Add custom order to get_feed
-- ============================================================
-- Adds p_custom_order_ids UUID[] to get_feed. When p_sort = 'custom'
-- and the array is non-null, ORDER BY uses array_position to follow
-- the client-specified order for matched rows, with unmatched rows
-- appended after (by created_at DESC).
--
-- This wires the previously-disconnected custom-sort feature:
-- feedStore.setCustomOrder set sort='custom' but buildFeedParams
-- never passed the order list, so the SQL could not apply it.
--
-- IMPORTANT: DROP the old 15-param signature first.
-- ============================================================
...
GRANT EXECUTE ON FUNCTION get_feed(
  UUID, TEXT, TEXT, UUID, UUID, UUID, UUID[], UUID[], UUID[], TEXT, TEXT, TIMESTAMPTZ, UUID, INTEGER, BOOLEAN, UUID[]
) TO authenticated;
