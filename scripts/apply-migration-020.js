...
async function applyMigration020() {
  ...
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      '../supabase/migrations/020_feed_view_mine_filter.sql'
    );
    ...
    const { rows } = await client.query(
      `SELECT proname, proargnames FROM pg_proc WHERE proname = 'get_visible_nodes'`
    );
    console.log('Function get_visible_nodes:', rows);
  } catch (e) {
