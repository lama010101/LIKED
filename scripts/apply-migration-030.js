...
async function applyMigration030() {
  ...
  try {
    const migrationPath = path.join(
      __dirname,
      'migrations/030_fix_rls_causes_edges.sql'
    );
    ...
    const { rows } = await client.query(`
      ...
      AND policyname IN (
        'causes_insert_policy',
        'causes_update_policy',
        'causes_delete_policy',
        'edges_insert_policy',
        'edges_update_policy',
        'edges_delete_policy'
      )
    `);
    console.log('Policies remaining:', rows);
    if (rows.length === 0) {
      console.log('✅ All 6 wide-open write policies successfully dropped');
    } else {
      console.log('❌ Some policies still exist:', rows);
    }
  } catch (e) {
